...
const ProfitabilityChart: React.FC<ProfitabilityChartProps> = ({ revenue, expense }) => {
  // Group revenue and expense data by month
  const combinedData = revenue.map((entry) => ({
    ...entry,
    type: "revenue",
    expense: expense.find((e) => e.selectedMonth === entry.selectedMonth)?.revenue || 0,
    difference: 0,
    prevMonthProfitability: 0,
  }));
